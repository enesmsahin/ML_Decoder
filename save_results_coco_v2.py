...
parser.add_argument('--allowed_classes_file', type=str, default="class-descriptions-boxable_nocaps.csv", help="Open Images CSV File indicating boxable classes")
...
prediction_tsv_files = (
    "/home/deepuser/deepnas/DISK4/DISK4/Enes/data/coco/coco_caption/train.label.tsv",
    "/home/deepuser/deepnas/DISK4/DISK4/Enes/data/coco/coco_caption/val.label.tsv",
    "/home/deepuser/deepnas/DISK4/DISK4/Enes/data/coco/coco_caption/test.label.tsv",
)
...
def main():
    ...
    # save_allowed_classes_list(f"allowed_classes_{len(classes_list_allowed)}.txt", classes_list_allowed)
    ...
    for prediction_tsv_file in tqdm(prediction_tsv_files):
        ...
        with open(prediction_tsv_file, "r") as pred_tsv_f:
            ...
            base_name = op.basename(prediction_tsv_file)
            base_name = "ml_decoder." + base_name
            # out_path = op.join(out_path, "labels")
            # os.makedirs(out_path)
            with open(op.join(out_path, base_name), "w") as out_label_tsv_f:
                out_line = ""
                lines = pred_tsv_f.readlines()
                for line in tqdm(lines):
                    img_id, curr_preds = line.split("\t")
                    img_path = next(Path(img_root_dir).rglob(f"*{img_id}*.jpg"))
                    try:
